...
def revert_coordinate_space(channels, R0, T0):
  ...
  R_diff = data_utils.expmap2rotmat(channels[rootRotInd])
  R = R_diff.dot(R0)
  channels_rec[rootRotInd] = data_utils.rotmat2expmap(R)
  T = T0 + ((R0.T).dot(np.reshape(channels[:3], [3,1]))).reshape(-1)
  channels_rec[:3] = T
  return channels_rec, R, T
